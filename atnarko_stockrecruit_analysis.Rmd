...
ggplot(SR_data, aes(spawners, recruits)) +
  geom_point(aes(colour = broodyear),size=4,show.legend=FALSE) +
  scale_colour_viridis_c(name = "Year")+geom_line(data=pred_df,aes(x=x_new,y=y_pred_d2),size=1.5)+
  theme(plot.margin = margin(1,2,1.3,1.2, "cm"))+xlab("Spawners") + ylab("Recruits")+theme(axis.text=element_text(size=11),
        axis.title=element_text(size=12,face="bold"))
```
...
ggplot(resid.df, aes(by, resids)) +
  ...
  scale_colour_viridis_c(name = "Year") + geom_ribbon(aes(ymin =resid.l90, ymax=resid.u90), alpha = 0.2) +xlab('Brood year')+ylab('Residual Productivity') + geom_hline(yintercept=0)
...
ggplot(resid.df, aes(by, resids)) +
  ...
  geom_point(aes(colour = by),size=3) +
  scale_colour_viridis_c(name = "Year",) + geom_ribbon(aes(ymin =resid.l90, ymax=resid.u90), alpha = 0.2) +xlab('Brood year')+ylab('Annual Productivity (log(R/S)) Deviation')
```
...
fg10a=ggplot(SR_data2, aes(cm_fry_lag, resids))+
  geom_point(aes(colour = year),size=2.5) +xlab('Chum fry')+ylab('Prod. deviations')+scale_colour_viridis_c(name = "Year")
 
fg10b=ggplot(SR_data2, aes(pk_avg, resids))+
  geom_point(aes(colour = year),size=2.5,show.legend = FALSE) +xlab('Pink abundance')+ylab(NULL)+scale_colour_viridis_c(name = "Year")

fg10c=ggplot(SR_data2, aes(SST_pine_yr, resids))+
  geom_point(aes(colour = year),size=2.5,show.legend = FALSE) +xlab('Annual SST anomaly')+ylab('Prod. deviations')+scale_colour_viridis_c(name = "Year")

fg10d=ggplot(SR_data2, aes(NPGO_2wint, resids))+
  geom_point(aes(colour = year),size=2.5,show.legend = FALSE) +xlab('Winter NPGO index')+ylab(NULL)+scale_colour_viridis_c(name = "Year")
...
f11a=ggplot(beta_cov, aes(x = beta_cov[,1])) + 
  ...
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "darkred",bins=30) +xlim(-1.5,1.5)+xlab('Effect size')+ggtitle("Chum fry releases")

f11b=ggplot(beta_cov, aes(x = beta_cov[,2])) + 
  geom_vline(xintercept=0)+
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "darkred",bins=30) +xlim(-1.5,1.5)+xlab('Effect size')+ggtitle("Pink abundance")

f11c=ggplot(beta_cov, aes(x = beta_cov[,3])) + 
  geom_vline(xintercept=0)+
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "darkred",bins=30) +xlim(-1.5,1.5)+xlab('Effect size')+ggtitle("SST anomaly")

f11d=ggplot(beta_cov, aes(x = beta_cov[,4])) + 
  geom_vline(xintercept=0)+
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "darkred",bins = 30) +xlim(-1.5,1.5)+xlab('Effect size')+ggtitle("NPGO")
